    ...
    if (transaction == null) {
      return const Scaffold(
        body: AppEmptyState(
            ...
            message: 'Aucune transaction liée à cette référence.'),
      );
    }
      ...
      body: ListView(
        ...
        children: [
          Container(
            ...
            child: Column(
              children: [
                ...
                _ReceiptRow(
                    label: 'Bénéficiaire',
                    value: transaction.beneficiary.fullName),
                _ReceiptRow(label: 'Corridor', value: transaction.corridor),
                _ReceiptRow(
                    label: 'Opérateur', value: transaction.operatorName),
                _ReceiptRow(
                ...
                _ReceiptRow(
                    label: 'Frais',
                    value:
                        '${transaction.fee.toStringAsFixed(0)} ${transaction.currency}'),
                _ReceiptRow(
                    label: 'Montant reçu',
                    value:
                        '${transaction.receivedAmount.toStringAsFixed(0)} ${transaction.currency}'),
                const SizedBox(height: 18),
                const Text('Signature numérique MiangPay',
                    style: TextStyle(
          ...
          SecondaryButton(
            ...
            onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Téléchargement préparé'))),
          ),
        ],
      ),
    );
